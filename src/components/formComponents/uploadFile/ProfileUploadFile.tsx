...
const ProfileUploadFile = ({
  ...
  return (
    <>
      <div className={`input-item w-[210px] h-[210px]   relative`}>
        <input
          type="file"
          id={id}
          style={{ display: "none" }}
        ...
        <div
          className={`flex flex-col items-start justify-center  h-full w-full rounded-10px border border-primaryBlack1 bg-primaryGreen cursor-pointer `}
        >
          {value && !isMulti && (
            <FileDisplay
          ...
          {(isMulti || !value) && (
            <div
              className="user-text font-FiraSansM show  flex items-center  px-1 flex-col justify-center w-full"
              onClick={() => {
                if (inputRef.current) inputRef.current.click();
              }}
            >
              <p className="user-text font-FiraSansM font-medium text-90px leading-[108px] text-white show">
                {letterText}
              </p>
...
export const FileDisplay = ({
  ...
  return (
    <div
      className={`${
        isMulti ? "w-[calc(100%_/_3)] px-2 h-[100px] mb-1" : "mx-auto h-full"
      }`}
    >
      <div
      ...
      >
        ...
        {type === EnumFileType.Image && source ? (
          <>
            <div
              className={`uploaded-data h-full group ${
                isMulti ? "bg-black rounded-xl" : "h-full "
